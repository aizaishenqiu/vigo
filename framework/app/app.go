...
func (app *App) Initialize() {
	...
	// 绑定 WebSocket 到 Benchmark 压力测试服务
	benchmarkSvc := benchmark.GetService()
	benchmarkSvc.SetHub(app.wsHub)
...
func (app *App) initMiddlewares() {
	...
	log.Log.Info("[Middleware] 日志中间件已启用")
...
func (app *App) Run(addr string) error {
	...
	// 自动加载安全中间件
	r.Use(middleware.RecoveryMiddleware())       // 错误恢复中间件
	r.Use(middleware.Logger())                   // 日志中间件
	r.Use(middleware.SecurityHeaders())          // 安全响应头
	r.Use(middleware.XSS())                      // XSS 防护
	r.Use(middleware.SQLInjection())             // SQL 注入防护
	r.Use(middleware.CSRF())                     // CSRF 防护
	r.Use(middleware.RequestSizeLimit(10 << 20)) // 请求大小限制（10MB）
	r.Use(middleware.AppErrorMiddleware())       // 应用错误处理中间件
	...
	// 初始化路由
	route.InitWithHub(r, app.wsHub)

	// 注册管理后台路由
	adminMgr := admin.GetManager()
	adminMgr.RegisterRoutes(r)
