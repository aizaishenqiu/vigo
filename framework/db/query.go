...
func (q *Query) Scan(dest interface{}) error {
	...
	if v.Elem().Kind() == reflect.Slice {
		// 使用 orm.go 中的 mapsToStructs 函数
		return mapsToStructs(results, dest)
	}

	// 单个结构体
	return mapToStruct(results[0], dest)
}
...
func (q *Query) query(executor *sql.DB, sqlStr string, args ...interface{}) ([]map[string]interface{}, error) {
	...
	// 记录查询日志
	elapsed := time.Since(start)
	GlobalQueryLogger.AddQuery(sqlStr, elapsed, args...)
...
func (q *Query) buildRawWhere(_ int) (string, []interface{}) {
	if len(q.rawWhere) == 0 {
		return "", nil
	}
